"use client";

import Link from "next/link";
import { useTranslations } from "next-intl";
import { useState } from "react";
import { usePwaInstall } from "@/components/providers/PwaProvider";
import { AboutDialog } from "./AboutDialog";
import { FeedbackDialog } from "./FeedbackDialog";

export function Footer() {
    const t = useTranslations("Footer");
    const [showAbout, setShowAbout] = useState(false);
    const [showFeedback, setShowFeedback] = useState(false);
    const [isInstalling, setIsInstalling] = useState(false);
    const { canInstall, isInstalled, isInstallSupported, promptInstall } = usePwaInstall();

    const handleInstallClick = async () => {
        if (canInstall) {
            setIsInstalling(true);
            try {
                await promptInstall();
            } finally {
                setIsInstalling(false);
            }
            return;
        }

        // Fallback guidance path when prompt is not available yet
        setShowAbout(true);
    };

    return (
        <>
            <footer className="site-footer">
                <div className="site-footer-inner">
                    <nav className="site-footer-links" aria-label="Footer links">
                        <Link
                            href="https://github.com/lAvArt/Quranic-Linguistics-Observatory"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="site-footer-link"
                            aria-label="GitHub"
                            style={{ display: "flex", alignItems: "center", padding: "6px 8px" }}
                        >
                            <svg
                                width="18"
                                height="18"
                                viewBox="0 0 24 24"
                                fill="currentColor"
                                aria-hidden="true"
                            >
                                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
                            </svg>
                        </Link>
                        {/* Standing rule: every page footer credits Kais Dukes
                            and the Quranic Arabic Corpus — the project this
                            app's morphology data comes from. */}
                        <Link
                            href="https://corpus.quran.com/"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="site-footer-link site-footer-credit"
                            title={t("creditTitle")}
                        >
                            {t("credit")}
                        </Link>
                        <Link
                            href="https://api-docs.quran.com/"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="site-footer-link site-footer-quranapi"
                        >
                            {t("quranApi")}
                        </Link>
                        <button
                            onClick={() => setShowAbout(true)}
                            className="site-footer-link about-link"
                        >
                            {t("about")}
                        </button>
                    </nav>
                    {isInstallSupported && !isInstalled && (
                        <button
                            onClick={handleInstallClick}
                            className="site-footer-link site-footer-install"
                            aria-label={t("installApp")}
                        >
                            {isInstalling ? t("installing") : t("installApp")}
                        </button>
                    )}
                    <button
                        onClick={() => setShowFeedback(true)}
                        className="site-footer-link site-footer-feedback"
                        aria-label={t("feedback")}
                    >
                        <svg
                            className="site-footer-feedback-icon"
                            width="18"
                            height="18"
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            aria-hidden="true"
                        >
                            <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z" />
                        </svg>
                        <span className="site-footer-feedback-text">{t("feedback")}</span>
                    </button>
                </div>
            </footer>

            <AboutDialog isOpen={showAbout} onClose={() => setShowAbout(false)} />
            <FeedbackDialog isOpen={showFeedback} onClose={() => setShowFeedback(false)} />
        </>
    );
}
